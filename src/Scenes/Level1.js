class Level1 extends Phaser.Scene {
    ...
    create() {
        ...
        //Rusher enemies
        // Walks slowly until the player comes within RUSHER_DETECT_RANGE px,
        // then bursts forward at RUSHER_CHARGE_SPEED. After RUSHER_CHARGE_TIME ms
        // it resets back to its patrol start and resumes slow walking.
        ...
        this.map.getObjectLayer("Object Layer 1").objects
            ...
            .forEach(obj => {
                ...
                enemy.spawnX        = obj.x;   // used to reset after charge
                enemy.spawnY        = obj.y;
                enemy.isCharging    = false;
                enemy.setVelocityX(40);        // slow walk speed
                enemy.body.immovable = true;
                enemy.enemyType = "rusher";
            });
 
        //Shooter
        // Stands still and fires a projectile toward the player every
        // SHOOTER_FIRE_INTERVAL ms.
        this.shooters = this.physics.add.group();
        this.projectiles = this.physics.add.group();   // projectile pool
        ...
        this.map.getObjectLayer("Object Layer 1").objects
            ...
            .forEach(obj => {
                ...
                // Fire on a repeating timer
                this.time.addEvent({
                    delay: 2200,          // ms between shots
                    loop: true,
                    callbackScope: this,
                    callback: () => {
    ...
    update() {
        ...
        this.rushers.getChildren().forEach(enemy => {
            if (enemy.isCharging) return;   // charge timer handles reset
            ...
            if (dist < RUSHER_DETECT_RANGE) {
                ...
                this.time.delayedCall(RUSHER_CHARGE_TIME, () => {
                    ...
                    // Teleport back to spawn and resume patrol
    ...
    fireProjectile(shooter) {
        ...
        proj.body.allowGravity = false;   // travels in a straight line
